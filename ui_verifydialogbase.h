/********************************************************************************
** Form generated from reading ui file 'verifydialogbase.ui'
**
** Created: Thu Nov 5 16:02:33 2009
**      by: Qt User Interface Compiler version 4.5.2
**
** WARNING! All changes made in this file will be lost when recompiling ui file!
********************************************************************************/

#ifndef UI_VERIFYDIALOGBASE_H
#define UI_VERIFYDIALOGBASE_H

#include <QtCore/QVariant>
#include <QtGui/QAction>
#include <QtGui/QApplication>
#include <QtGui/QButtonGroup>
#include <QtGui/QDialog>
#include <QtGui/QHBoxLayout>
#include <QtGui/QHeaderView>
#include <QtGui/QLabel>
#include <QtGui/QPushButton>
#include <QtGui/QSpacerItem>
#include <QtGui/QVBoxLayout>

QT_BEGIN_NAMESPACE

class Ui_VerifyDialogBase
{
public:
    QVBoxLayout *vboxLayout;
    QHBoxLayout *hboxLayout;
    QLabel *pixmapLabel1;
    QLabel *m_textLabel;
    QHBoxLayout *hboxLayout1;
    QSpacerItem *Horizontal_Spacing2;
    QPushButton *buttonYes;
    QPushButton *buttonNo;
    QPushButton *buttonCancel;

    void setupUi(QDialog *VerifyDialogBase)
    {
        if (VerifyDialogBase->objectName().isEmpty())
            VerifyDialogBase->setObjectName(QString::fromUtf8("VerifyDialogBase"));
        VerifyDialogBase->resize(360, 119);
        VerifyDialogBase->setWindowIcon(qt_get_icon(image0_ID));
        VerifyDialogBase->setSizeGripEnabled(false);
        VerifyDialogBase->setModal(true);
        vboxLayout = new QVBoxLayout(VerifyDialogBase);
        vboxLayout->setSpacing(6);
        vboxLayout->setMargin(11);
        vboxLayout->setObjectName(QString::fromUtf8("vboxLayout"));
        hboxLayout = new QHBoxLayout();
        hboxLayout->setSpacing(6);
        hboxLayout->setObjectName(QString::fromUtf8("hboxLayout"));
        pixmapLabel1 = new QLabel(VerifyDialogBase);
        pixmapLabel1->setObjectName(QString::fromUtf8("pixmapLabel1"));
        QSizePolicy sizePolicy(static_cast<QSizePolicy::Policy>(0), static_cast<QSizePolicy::Policy>(0));
        sizePolicy.setHorizontalStretch(0);
        sizePolicy.setVerticalStretch(0);
        sizePolicy.setHeightForWidth(pixmapLabel1->sizePolicy().hasHeightForWidth());
        pixmapLabel1->setSizePolicy(sizePolicy);
        pixmapLabel1->setMinimumSize(QSize(50, 50));
        pixmapLabel1->setMaximumSize(QSize(50, 50));
        pixmapLabel1->setPixmap(qt_get_icon(image0_ID));
        pixmapLabel1->setScaledContents(true);
        pixmapLabel1->setWordWrap(false);

        hboxLayout->addWidget(pixmapLabel1);

        m_textLabel = new QLabel(VerifyDialogBase);
        m_textLabel->setObjectName(QString::fromUtf8("m_textLabel"));
        m_textLabel->setWordWrap(false);

        hboxLayout->addWidget(m_textLabel);


        vboxLayout->addLayout(hboxLayout);

        hboxLayout1 = new QHBoxLayout();
        hboxLayout1->setSpacing(6);
        hboxLayout1->setObjectName(QString::fromUtf8("hboxLayout1"));
        Horizontal_Spacing2 = new QSpacerItem(210, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        hboxLayout1->addItem(Horizontal_Spacing2);

        buttonYes = new QPushButton(VerifyDialogBase);
        buttonYes->setObjectName(QString::fromUtf8("buttonYes"));
        buttonYes->setAutoDefault(true);
        buttonYes->setDefault(true);

        hboxLayout1->addWidget(buttonYes);

        buttonNo = new QPushButton(VerifyDialogBase);
        buttonNo->setObjectName(QString::fromUtf8("buttonNo"));
        buttonNo->setAutoDefault(true);

        hboxLayout1->addWidget(buttonNo);

        buttonCancel = new QPushButton(VerifyDialogBase);
        buttonCancel->setObjectName(QString::fromUtf8("buttonCancel"));
        buttonCancel->setAutoDefault(true);

        hboxLayout1->addWidget(buttonCancel);


        vboxLayout->addLayout(hboxLayout1);


        retranslateUi(VerifyDialogBase);
        QObject::connect(buttonYes, SIGNAL(clicked()), VerifyDialogBase, SLOT(yesSlot()));
        QObject::connect(buttonCancel, SIGNAL(clicked()), VerifyDialogBase, SLOT(cancelSlot()));
        QObject::connect(buttonNo, SIGNAL(clicked()), VerifyDialogBase, SLOT(noSlot()));

        QMetaObject::connectSlotsByName(VerifyDialogBase);
    } // setupUi

    void retranslateUi(QDialog *VerifyDialogBase)
    {
        VerifyDialogBase->setWindowTitle(QApplication::translate("VerifyDialogBase", "Linguistica : ", 0, QApplication::UnicodeUTF8));
        m_textLabel->setText(QString());
        buttonYes->setText(QApplication::translate("VerifyDialogBase", "&Yes", 0, QApplication::UnicodeUTF8));
        buttonYes->setShortcut(QApplication::translate("VerifyDialogBase", "Alt+Y", 0, QApplication::UnicodeUTF8));
#ifndef QT_NO_TOOLTIP
        buttonYes->setProperty("toolTip", QVariant(QString()));
#endif // QT_NO_TOOLTIP
#ifndef QT_NO_WHATSTHIS
        buttonYes->setProperty("whatsThis", QVariant(QApplication::translate("VerifyDialogBase", "Yes, do it.", 0, QApplication::UnicodeUTF8)));
#endif // QT_NO_WHATSTHIS
        buttonNo->setText(QApplication::translate("VerifyDialogBase", "&No", 0, QApplication::UnicodeUTF8));
        buttonNo->setShortcut(QApplication::translate("VerifyDialogBase", "Alt+N", 0, QApplication::UnicodeUTF8));
#ifndef QT_NO_WHATSTHIS
        buttonNo->setProperty("whatsThis", QVariant(QApplication::translate("VerifyDialogBase", "No, don't do it.", 0, QApplication::UnicodeUTF8)));
#endif // QT_NO_WHATSTHIS
        buttonCancel->setText(QApplication::translate("VerifyDialogBase", "&Cancel", 0, QApplication::UnicodeUTF8));
        buttonCancel->setShortcut(QString());
#ifndef QT_NO_WHATSTHIS
        buttonCancel->setProperty("whatsThis", QVariant(QApplication::translate("VerifyDialogBase", "Cancel ... do nothing.", 0, QApplication::UnicodeUTF8)));
#endif // QT_NO_WHATSTHIS
        Q_UNUSED(VerifyDialogBase);
    } // retranslateUi


protected:
    enum IconID
    {
        image0_ID,
        unknown_ID
    };
    static QPixmap qt_get_icon(IconID id)
    {
    static const unsigned char image0_data[] = { 
    
};

    switch (id) {
        case image0_ID:  { QImage img; img.loadFromData(image0_data, sizeof(image0_data), "PNG"); return QPixmap::fromImage(img); }
        default: return QPixmap();
    } // switch
    } // icon

};

namespace Ui {
    class VerifyDialogBase: public Ui_VerifyDialogBase {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_VERIFYDIALOGBASE_H
